...
vignette assumes users have already setup user access tokens (see: the "auth" vignette, `vignette("auth", package = "rtweet")`).
...
stream <- stream_tweets(q = q, timeout = streamtime, file_name = filename)
```
...
at a later point in time is the same, as illustrated in the example
below.
