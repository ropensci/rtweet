...
Next we need to authenticate as it:

```{r}
auth_as("default")
```

Then we are ready to use the package.
...
```{r}
path_file <- ggplot2::ggsave(tempfile(fileext = ".png"), plot)
post_tweet("my first rtweet #rstats", media = path_file, media_alt_text = "Plot of the R Foundation monthly tweets since October 2017. At most 7 tweets per month, with some months without tweets.")
```
...
```{r, eval=FALSE}
## get user IDs of accounts followed by R Foundation
tmls <- get_timeline("_R_Foundation", n = 3200)
