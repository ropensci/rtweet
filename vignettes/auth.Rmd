...
This mechanism is very simple to set up, but is best suited for casual use of rtweet.

If you want to collect a lot of data or implement a bot, you should instead use one of the two other authentication mechanisms provided by rtweet:

-   **App authentication** allows you to act as if you were a Twitter app.
    You can't perform operations that a user can (like posting a tweet or reading a DM), but in return you get higher rate limits on operations like searching for tweets.
...
Next, you'll learn how to use the `rtweet_app()` and `rtweet_bot()` functions to store your app configuration so rtweet can use them when requesting data from the API.
...
You're already familiar with using twitter, either through [the website](https://twitter.com) or one of a variety of apps you can install on your phone or computer.
Now you'll have to learn more about what's going on behind the scenes.
...
You'll call either `rtweet_app()` or `rtweet_bot()` depending on whether you want app-style authentication or bot-style authentication as described above.
