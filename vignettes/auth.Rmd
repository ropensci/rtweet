...
This vignette covers how create your own Twitter developer app if you want towant to post tweets, use a bearer token, or read/write direct messages, or work non-interactively. 
...
Users can create their personal Twitter token in two different ways. The first method is suitable if you want to allow users to interactively login using their own accounts; the second is appropriate for non-interactive environments.
...
This method is appropriate if you want your app to 
...
`create_token()` will automatically cache the token for you, so the next time you start an R session [on the same machine], rtweet should automatically find your token. To double check it works, restart your R session, run the following code, and check that the app name and `api_key` match.
