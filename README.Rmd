...
+ See the `auth` vignette for instructions on obtaining access to Twitter's APIs: [http://rtweet.info/articles/auth.html](http://rtweet.info/articles/auth.html).

### API authorization
+ To access Twitter's APIs, go to [apps.twitter.com](https://apps.twitter.com/)
...
+ Click on tab labeled `Keys and Access Tokens` 
   - Copy consumer (api) and secret keys and paste (along with app name) into R script
   - Use `create_token()` to save token as an environment variable (so you don't have to worry about doing this process again in future R sessions).
