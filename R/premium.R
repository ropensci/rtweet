...
#' @param bucket Unit of time the count data will be provided. Options, one of: 
#' "day", "hour", "minute".
#' @param continue A character string with the next results of a query. You 
#' must make the exact same query as the original, including `q`, `toDate`, 
#' `fromDate` and `bucket`.
#' @return A tibble data frame of Twitter data for each unit of bucket.
...
#'
#' @export
#' @references <https://developer.twitter.com/en/docs/twitter-api/premium/search-api/api-reference/premium-search#CountsEndpoint>
counts_fullarchive <- function(q, n = 100, fromDate = NULL, toDate = NULL,
                               continue = NULL, bucket = NULL,
                               env_name = NULL, premium = FALSE, 
                               safedir = NULL, parse = TRUE, token = NULL) {
  
  counts_premium("fullarchive", 
                q = q, 
                n = n,
                fromDate = fromDate,
                toDate = toDate,
...
#' @rdname counts_fullarchive
#' @export
counts_30day <- function(q, n = 100, fromDate = NULL, toDate = NULL,
                        env_name = NULL, 
                        continue = NULL,  
                        bucket = NULL,
                        premium = FALSE,
...
counts_premium <- function(product, q, n = NULL, fromDate = NULL, toDate = NULL,
                          env_name = NULL, continue = NULL, 
                          bucket = "hour", premium = FALSE, safedir = NULL,
                          parse = TRUE,
                          token = NULL) {
  ...
  bucket <- match.arg(bucket, c("day", "hour", "minute"))
  ...
  api <- paste0("/1.1/tweets/search/", product, "/", env_name, "/counts")
  result <- TWIT_paginate_premium(token, api, params, n = n, cursor = continue,
                                  page_size = if (premium) 500 else 100)
