...
#'
#' @param token Oauth token created via \code{\link{create_token}}. See details for more information on valid tokens.
#' @return A bearer token
#' @details \code{bearer_token()} will only work on valid tokens generated from a user-created Twitter app (requires a Twitter developer account; see
#'   \code{\link{create_token}} for more information). Unlike the default token returned by \code{create_token}, bearer tokens operate without any knowledge/information about the user context–meaning, bearer token requests cannot engage in user actions (e.g., posting tweets, reading DMs) and the information returned by Twitter will not include user-specific variables (e.g., if the user is following a certain account). The upside to this authentication method is that it can afford users with more generous rate limits. For example, the rate limit for the standard search API is 18,000 tweets per fifteen minutes. With a bearer token, the rate limit is 45,000 tweets per fifteen minutes. However, this is not true for all endpoints. For a breakdown/comparison of rate limits, see \href{https://developer.twitter.com/en/docs/basics/rate-limits.html}.
#' @examples
#' \dontrun{
