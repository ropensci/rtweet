...
ts_plot <- function(rt,
  ...
  axis(2, at = round(seq(ylim[1], ylim[2], length.out = 4), 0),
    lwd = 2, col = "#666666",
  ...
  for (i in seq_along(df)) {
    lo <- mean(df[[i]]$freq, na.rm = TRUE) -
      (sd(df[[i]]$freq, na.rm = TRUE) * .1)
    if (all(
      df[[i]]$freq[1] < lo,
      df[[i]]$freq[NROW(df[[i]])] < lo)) {
      df[[i]] <- df[[i]][2:(NROW(df[[i]]) - 1), ]
    }
    x <- df[[i]][, 1:2]
    ...
    text(leg.x, leg.y, labels = leg.text,
      col = paste0(cols[i], "ff"), cex = runif(1, .95, 1.3))
  }
  par(op)
