...
ts_plot <- function(rt, by = "days", txt = "text", filter = NULL,
    ...
    par(bg = "#f0f0f0", bty = "n", tcl = -.125, las = 1,
        mar = c(5, 4, 4, 3))
    grid.minor <- rep(0, 49)
    ...
    plot(df$time, df$freq, ylim = yaxis[c(1, 4)], type = "p",
         ...
         panel.first = list(rect(par("usr")[1], par("usr")[3],
                            ...
                            panel.last = grid(4, 4, lty = grid.major,
                                              lwd = .75),
...
#' @export
rtline <- function(., new = FALSE, ...) {
    names(.)[1:2] <- c("time", "freq")
    .$time <- as.numeric(.$time)
    .$freq <- as.numeric(.$freq)
    if (new) {
        with(., plot(time, freq, type = "l", lty = lty, ...))
    } else {
        with(., lines(time, freq, ...))
    }
    invisible(.)
}
